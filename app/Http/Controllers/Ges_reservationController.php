...
class Ges_reservationController extends Controller
{
    /**
    ...
    {
        $data_ges_reserv = Ges_reservation::all();
        //dd($data_ges_reserv);
        ...
        $request->validate(
           ...
           //  dd($request);
            try{
                ...
                Ges_reservation::create(array_merge($request->all()));
                DB::commit();
                return view('/reservation');
            }catch(\Throwable $th){
              DB::rollBack();
    ...
    public function destroy(string $ref)
    {
      //  dd($id);
        if($ref){
          ...
          $res = Ges_reservation::where("reference", $ref)->delete();
    ...
    {
        $res = Ges_reservation::where('reference', $ref)->update(['statut' => "encour"]);
       // dd($ref);
        if($res)
        {
